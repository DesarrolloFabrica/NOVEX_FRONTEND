# O.M.E.G.A. — Dirección de Arte: estado actual
...
```
OmegaRoom          → escenario exterior (fondo, profundidad, viñeta)
  OmegaFrame       → marco metálico de gran pantalla operativa
...
|---|---|---|
| `OmegaRoom` | `src/modules/room/components/OmegaRoom.tsx` | Suelo, profundidad radial, iluminación ambiental reactiva |
| `OmegaFrame` | `src/modules/room/components/OmegaFrame.tsx` | Chasis metálico, bisel, sombra proyectada |
...
- Consumo: `OmegaRoom`, `OmegaFrame`, `MainScreen`, `ScreenDeck`, consolas, paneles, plataforma
...
- Accesibilidad: `@media (prefers-reduced-motion: reduce)` desactiva todas las animaciones `omega-breath-*`
